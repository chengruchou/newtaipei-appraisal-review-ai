...
[ADR 0051](adr/0051-workbook-pdf-conversion.md).
...
| Change | Purpose |
| --- | --- |
| `adapters/aws/routing_discovery.py` | Read-only, dispatch-guarded discovery of a model's complete destination set, returning a reproducible `destination_snapshot_sha256`. Gives `check_model_destinations` a producer. See [ADR 0048](adr/0048-observed-model-routing-evidence.md) |
...
| `domain/source_manifest.py`, `configs/sources/newtaipei-shulin-1110901.json`, `scripts/check_case_sources.py` | Declared case sources, subject roles, effective date, per-source handling scope and open questions, plus a checker that measures supplied originals without pinning them. See [ADR 0049](adr/0049-declared-case-source-manifest.md) |
| `adapters/local/case_sources.py` | The direct non-privacy path: pinned originals to a parsed `SourceRegistry` through the existing native parser. See [ADR 0050](adr/0050-direct-non-sensitive-source-path.md) |
| `ports/workbook_conversion.py`, `adapters/local/workbook_pdf.py` | Faithful workbook to PDF rendering with an honest capability report and per-run output verification. See [ADR 0051](adr/0051-workbook-pdf-conversion.md) |
