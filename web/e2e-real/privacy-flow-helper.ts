...
import {
  completeLocalOcrReview,
  requiresOcrReview,
  pauseLocalOcrReview,
} from "./ocr-review-helper";
import type {
  ServiceResult,
  TaskListView,
  TaskSubjectView,
...
async function originalPrivacyFlow(page: Page, mode: "complete" | "paused") {
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    await expect
      .poll(
        async () => {
          const jobResponse = await rehearsalGet(reviewer.request, `/v1/review-jobs/${jobId}`, {
          ...
          const response = await rehearsalGet(reviewer.request, `/v1/review-jobs/${jobId}/tasks`, {
            headers: backendHeaders,
          });
          expect(response.ok()).toBe(true);
    ...
    const metadata = await rehearsalGet(
      reviewer.request,
      `/v1/review-tasks/${task.task_id}/subject`,
      {
        headers: backendHeaders,
      },
    );
  ...
  const resultResponse = await rehearsalGet(reviewer.request, `/v1/review-jobs/${jobId}/result`, {
    headers: backendHeaders,
  });
  ...
  if (mode === "paused") {
    const pending = page.waitForResponse(
      (response) =>
        response.url() === `${prefix}/restore/${handoff.restore_result_id!}` &&
        response.request().method() === "POST",
      { timeout: 125_000 },
    );
    await page
      ...
      .click();
    await pauseLocalOcrReview(
      page,
      {
        ...fixture,
        restore_result_id: handoff.restore_result_id!,
        completed_job_id: jobId,
      },
      await pending,
      artifact.content_hash,
    );
    const unauthorized = await rehearsalGet(page.request, `${prefix}/sources`, {
...
export async function runPrivacyFlow(page: Page, mode: "complete" | "paused") {
  const path = process.env["PRIVACY_BROWSER_FIXTURE"];
  if (!path) throw new Error("Configure the exact private rehearsal fixture.");
  let fixture: PrivacyFixture;
  try {
    fixture = JSON.parse(readFileSync(path, "utf8")) as PrivacyFixture;
  } catch {
    throw new Error("The private rehearsal fixture could not be read.");
  }
  ...
  try {
    await originalPrivacyFlow(page, mode);
  } finally {
    finish();
  }
}
